...
export default function InvestorLayout({ children }) {
  ...
  const menuItems = [
    ...
    { path: '/investor/properties', label: 'Properties', icon: BuildingOfficeIcon },
    { path: '/investor/geospatial', label: 'Geospatial', icon: MapIcon },
    ...
    { path: '/investor/profile', label: 'Profile', icon: UserIcon },
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--surface)', fontFamily: 'var(--ff-body)', display: 'flex' }}>
      ...
      {/* ── Sidebar ── */}
      <aside className={`rw-inv-sidebar ${sidebarOpen ? 'rw-inv-sidebar--open' : ''} ${sidebarCollapsed ? 'rw-inv-sidebar--collapsed' : ''}`}>

        {/* Logo */}
        <div className="rw-inv-sidebar__header">
          {!sidebarCollapsed && (
          ...
          {sidebarCollapsed && (
            <div className="rw-tl-logo__mark" style={{ margin: '0 auto' }}>
              <HomeIcon style={{ width: 18, height: 18, color: '#fff' }} />
            </div>
          )}
          <button
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.path}
                onClick={() => setSidebarOpen(false)}
                className={`rw-inv-nav-item ${isActive(item.path) ? 'rw-inv-nav-item--active' : ''} ${sidebarCollapsed ? 'rw-inv-nav-item--collapsed' : ''}`}
              ...
              >
                ...
                {!sidebarCollapsed && (
                  <>
                    ...
                    {item.badge ? (
                      <span className="rw-tl-badge">{item.badge}</span>
                    ) : null}
                  </>
                )}
              </Link>
          ...
          <div style={{ padding: 14, borderTop: '1px solid var(--border)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ fontSize: 10, color: 'var(--text-muted)' }}>Investor</p>
              </div>
              <button
        ...
        >
          <div style={{ padding: '0 32px', height: 64, display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <p style={{ fontFamily: 'var(--ff-display)', fontSize: 18, fontWeight: 700, color: 'var(--ink)' }} className="rw-ll-page-title">
              {menuItems.find((m) => isActive(m.path))?.label || 'Investor Dashboard'}
            </p>

            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginLeft: 'auto' }}>
              {/* Premium badge */}
              <span style={{ padding: '5px 14px', background: 'rgba(200,169,110,0.12)', border: '1px solid rgba(200,169,110,0.3)', borderRadius: 100, fontSize: 11, fontWeight: 700, color: 'var(--accent)', letterSpacing: '0.06em' }} className="rw-ll-username">
                Investor Pro
              </span>
                  ...
                  {userMenuOpen && (
                    <>
                      ...
                      <div className="rw-tl-dropdown">
                        <Link href="/investor/profile" className="rw-tl-dropdown__item" onClick={() => setUserMenuOpen(false)}>Profile</Link>
                        <Link href="/investor/settings" className="rw-tl-dropdown__item" onClick={() => setUserMenuOpen(false)}>Settings</Link>
      ...
      <style jsx global>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .rw-inv-sidebar--collapsed { width: 68px; }
        ...
        @media (max-width: 1024px) {
          ...
          .rw-inv-sidebar--open { transform: translateX(0); }
        }

        .rw-inv-sidebar__header {
        ...
        .rw-inv-nav-item:hover { background: rgba(200,169,110,0.07); color: var(--accent); }
        .rw-inv-nav-item--active { background: rgba(200,169,110,0.13); color: var(--accent); }
        .rw-inv-nav-item--collapsed { justify-content: center; padding: 11px 0; }
        ...
        @media (max-width: 1024px) { .rw-inv-hamburger { display: flex; } }
